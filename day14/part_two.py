...
def is_legal(sand_coords):
    return sand_coords[1] <= max_y


is_touching_top = False
while not is_touching_top:
    if go_down(current_sand_coords) not in obstacles and is_legal(current_sand_coords):
        current_sand_coords = go_down(current_sand_coords)
        continue
    elif go_left_down(current_sand_coords) not in obstacles and is_legal(current_sand_coords):
        current_sand_coords = go_left_down(current_sand_coords)
        continue
    elif go_right_down(current_sand_coords) not in obstacles and is_legal(current_sand_coords):
        current_sand_coords = go_right_down(current_sand_coords)
        continue
    else:
        if current_sand_coords == sand_starting_point:
